...
//		PostOrder(root);
	...
	int sumHelper(BSTNode<E> node) {
		...
		if(node == null)
			return 0;
		int sum = (int)node.info;
		sum += sumHelper(node.left);
		sum += sumHelper(node.right);
		return sum;
	}
	public int height(){
	...
	int heightHelper(BSTNode<E> node) {
		...
		if(node == null)
			return -1;
		int height = 1;
		int leftMax = heightHelper(node.left);
		int rightMax = heightHelper(node.right);
		if(leftMax > rightMax) {
			height += leftMax;
		} else {
			height += rightMax;
		}
		return height; // change this as needed
	}
}
